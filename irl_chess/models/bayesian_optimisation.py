...
from irl_chess.chess_utils.sunfish_utils import sunfish_move_to_str, get_new_pst, moves_and_Q_from_result
...
def run_bayesian_optimisation(sunfish_boards, config_data, out_path, player_moves):
    ...
    with (Parallel(n_jobs=config_data['n_threads']) as parallel):
        ...
        actions_true = [sunfish_move_to_str(move) for move in actions_true]
        ...
        def objective_function(x):
            ...
            actions_new, Q_new_dicts = moves_and_Q_from_result(results, sunfish_boards)
        ...
        for epoch in range(epochs):
            ...
            opt.run_optimization(max_iter=1, verbosity=config_data['optimisation_verbosity'], eps=0)
...
def bayesian_model_result_string(model_config_data):
    ...
    return f"{delta}--{time_limit}--{R_start}-{R_true}"
