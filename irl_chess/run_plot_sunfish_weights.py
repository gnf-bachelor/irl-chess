...
def run_loop(plot_weights, plot_accuracies, move_functions, elos, extensions=('', ), out_path=None, save_path=None, out_path_def_sunfish=None):
    ...
    for move_function in move_functions:
        ...
        for extension in extensions:
            ...
            for i, elo in enumerate(elos):
                ...
                if plot_weights:
                    save_path = join(os.getcwd(), 'results', 'plots', 'data_section', f'trace_plot_{move_function}_{elos[0]}_{elos[-1]}.svg') if save_path is None else save_path
                    try:
                ...
                if plot_accuracies:
                    save_path = join(os.getcwd(), 'results', 'plots', 'data_section',
                                     f'trace_plot_accuracies_{move_function}__{elos[0]}-{elos[-1]}.svg')
                    ...
                    try:
                        ...
                        if default_acc[i] is None and 'player' in move_function:
                            ...
                            default_acc[i], _ = val_sunfish_GRW(validation_set=validation_set,
                                                             out_path=os.path.dirname(default_weight_path) if out_path_def_sunfish is None else out_path_def_sunfish,
...
if __name__ == '__main__':
    ...
    options = ['gpw_8', 'SunfVal', 'alpha-beta', 'BIRLall', ]
    opt_index = 1
                ...
                for move_function in move_functions:
                    ...
                    for filename in os.listdir(path_run):
                        if os.path.isdir(join(path_run, filename)) and not filename.endswith('plots'):
                            save_path = join(path_run, 'plots', f'trace_plot_pieces_{move_function}_{elo}_{"weights" if do_weights else "accuracies"}.svg')
