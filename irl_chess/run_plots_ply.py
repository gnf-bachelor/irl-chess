...
from irl_chess.data import load_maia_test_data
...
def run_comparison(run_sunfish=False, pgn_paths=None, move_range=(10, 100), val_proportion=0.2, sunfish_epoch=90,
    ...
    os.makedirs(out_path_sunfish, exist_ok=True)
    ...
    acc_sunfish_def_list = []
    acc_sunfish_list = []
    acc_maia_list = []
    acc_random_list = []
    lower_bound_sunfish_def_list = []
    upper_bound_sunfish_def_list = []
    ...
    for n_moves in tqdm(range(move_range[0] + offset, move_range[1]), desc='Iterating over n_moves'):
        ...
        out_path_sunfish_def = join(os.path.dirname(out_path_sunfish), 'default_sunfish')
        default_weight_path = join(out_path_sunfish_def, 'weights')
        ...
        acc_sunfish_def, (lower_bound_sunfish_def, upper_bound_sunfish_def) = val_sunfish_GRW(
            epoch=0,
            use_player_moves=True,
            config_data=config_data_sunfish,
            out_path=out_path_sunfish_def,
            validation_set=validation_set,
            name=f'n_moves_{base_config_data["min_elo"]}_{base_config_data["max_elo"]}/{n_moves}'
        ...
        acc_sunfish_def_list.append(acc_sunfish_def)
        acc_random_list.append(acc_random)
        acc_sunfish_list.append(acc_sunfish)
        acc_maia_list.append(acc_maia)
        lower_bound_sunfish_def_list.append(lower_bound_sunfish_def)
        upper_bound_sunfish_def_list.append(upper_bound_sunfish_def)
        ...
        while time() - t < 1:
            sleep(.1)
        plot_ply(acc_lists, plot_path, run_sunfish, move_range, n_moves, config_data_maia, config_data_sunfish, csv_path)

        print(f'Using sunfish with following config:')
        print(config_data_sunfish)
    plot_path = join(plot_path_base, f'{move_range[1]-1}.svg')
    plot_ply(acc_lists, plot_path, run_sunfish, move_range, move_range[1]-1, config_data_maia, config_data_sunfish, csv_path)


def plot_ply(acc_lists, plot_path, run_sunfish, move_range, n_moves, config_data_maia, config_data_sunfish, csv_path):
    [acc_random_list,
     lower_bound_random_list,
     upper_bound_random_list,
     acc_sunfish_list,
     ...
     acc_maia_list,
     lower_bound_maia_list,
     upper_bound_maia_list] = acc_lists
    alpha = 0.15
    ...
    if run_sunfish:
        plt.plot(range(move_range[0], n_moves + 1), acc_sunfish_list,
                 label=f'Sunfish {config_data_sunfish["min_elo"]}', color=sunfish_palette[0])
        plt.fill_between(range(move_range[0], n_moves + 1), lower_bound_sunfish_list, upper_bound_sunfish_list,
                         alpha=alpha, color=sunfish_palette[0])
        plt.plot(range(move_range[0], n_moves + 1), acc_sunfish_def_list,
                 label=f'Sunfish Default', color=sunfish_palette[1])
        plt.fill_between(range(move_range[0], n_moves + 1), lower_bound_sunfish_def_list, upper_bound_sunfish_def_list,
                         alpha=alpha, color=sunfish_palette[1])

    plt.plot(range(move_range[0], n_moves + 1), acc_maia_list,
             label=f'Maia {config_data_maia["maia_elo"]}', color=maia_palette[0])
    plt.fill_between(range(move_range[0], n_moves + 1), lower_bound_maia_list, upper_bound_maia_list,
                     alpha=alpha, color=maia_palette[0])

    plt.plot(range(move_range[0], n_moves + 1), acc_random_list, label='Random', color=random_palette[-1])
    plt.fill_between(range(move_range[0], n_moves + 1), lower_bound_random_list, upper_bound_random_list,
                     alpha=alpha, color=random_palette[-1])

    plt.title(
        f'Sunfish Default and {config_data_sunfish["min_elo"]} vs Maia {config_data_maia["maia_elo"]} \nAccuracy from {move_range[0]} to {n_moves} moves into a game')
    plt.xlabel('Number of plys into game')
    plt.ylabel('Accuracy')
    ...
    df_out = pd.DataFrame(
        ...
        columns=['random', 'random_lower', 'random_upper', 'sunfish', 'sunfish_lower', 'sunfish_upper', 'sunfish_def', 'sunfish_def_lower', 'sunfish_def_upper', 'maia',
...
if __name__ == '__main__':
    pgn_paths =None # ['data/raw/lichess_db_standard_rated_2013-01.pgn']
    ply_range = (10, 81)
