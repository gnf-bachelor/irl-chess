...
if __name__ == '__main__':
    from irl_chess import download_file, load_maia_network
    ...
    for elo_maia in tqdm(range(maia_range[0], maia_range[1], 100), desc='ELO Maia'):
        ...
        for elo_player in tqdm(range(player_range[0], player_range[1], 100), desc='ELO Players'):
            ...
            val_df = val_df[(10 <= val_df['move_ply'])][:n_moves]
            acc_sum = 0
            # page 4 of the paper states that moves with less than 30 seconds left have been discarded
            for move_true, fen in tqdm(zip(val_df['move'], val_df['board']), desc='Moves', total=n_moves):
        ...
        plot_accuracies(accuracies, elos_players, maia_elos, n_moves=n_moves)
        ...
        df_out.to_csv(f'results/maia_replication/{maia_elos[0]}_{maia_elos[-1]}_{n_moves}.csv')
