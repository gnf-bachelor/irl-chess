...
def plot_accuracies_over_elo(accuracies, time_limits, elos, n_boards, save_filename, legend_title='Seconds'):
...
def plot_depth(base_config, model_config, elos, depths):
    accuracies = [[] for el in depths]
    ...
    for j, depth in tqdm(enumerate(depths), desc='Depths', total=len(depths)):
        for i, elo in tqdm(enumerate(elos), desc='ELOs', total=len(elos)):
            base_config['min_elo'] = elo
            base_config['max_elo'] = elo + 100
            base_config['move_function'] = 'player_move'
            ...
            base_config['alpha'] = .5
            base_config['add_legend'] = True
            base_config['plot_title'] = (f'Sunfish default weights for different ELOs and depths limits')
            ...
            acc, _ = val_sunfish_GRW(
                ...
                config_data=union_dicts(model_config, base_config),
                epoch=0,
                name=f'depth-{base_config["n_boards"]}-{depth}-{elo}',
                use_player_moves=True)
            accuracies[j].append(acc)

        save_filename = join(os.path.dirname(out_path), 'plots', 'depth', f'depths_{depths[0]}-{depth}-{elos[0]}-{elo}.svg')
        plot_accuracies_over_elo(accuracies, depths, elos, base_config['n_boards'], save_filename, legend_title='Depth')
...
if __name__ == '__main__':
    ...
    elos = range(1100, 2000, 100)
    depths = [1, 2, 3, 4, 5, 6, 7, ]
    depths.reverse()
    plot_depth(*configs, depths=depths, elos=elos)

    #time_limits = [0.01, 0.1, 0.2, 0.4, 0.75, 1., 1.5,]# 2.]
    #time_limits.reverse()
    #plot_time_limit(*configs, elos=elos, time_limits=time_limits)
